...
export default function ReportFeed({ reports, onFlag, onConfirm, onResolve, onEscalate, onVoteResolve, focused }) {
    ...
    <div className="feed">
      {list.map((r) => (
        <ReportCard key={r.id} report={r} onFlag={onFlag} onConfirm={onConfirm} onResolve={onResolve}
          onEscalate={onEscalate} onVoteResolve={onVoteResolve} highlight={focused && r.id === focused.id} />
      ))}
    </div>
  );
}
